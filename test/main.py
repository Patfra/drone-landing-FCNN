...
def load_vgg(sess, vgg_path):
    ...
    # conv6
    # tf.name_scope = 'conv6'
    kernel = tf.Variable(tf.truncated_normal([11, 20, 512, 32], dtype=tf.float32,
                                             stddev=1e-1), name='conv6/weights')
    conv = tf.nn.conv2d(pool5, kernel, [1, 1, 1, 1], padding='VALID')
    biases = tf.Variable(tf.constant(0.0, shape=[32], dtype=tf.float32),
                         trainable=True, name='conv6/biases')
    out = tf.nn.bias_add(conv, biases)
    ...
    # conv7
    # tf.name_scope = 'conv7'
    kernel = tf.Variable(tf.truncated_normal([1, 1, 32, 4096], dtype=tf.float32,
                                             stddev=1e-1), name='conv6/weights')
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
             correct_label, keep_prob, learning_rate, correct_labels, logits):
    """
    ...
    for epoch in range(epochs):
        ...
        for X_batch, gt_batch in get_batches_fn(batch_size):
            loss, _, cl, lo = sess.run([cross_entropy_loss, train_op, correct_labels, logits],
